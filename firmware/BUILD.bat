...
ECHO *** Building %%A.c...
sdcc --model-small -mmcs51 -pdefcpu -c -obin\%%A.rel "code\\"%%A.c
...
copy /y templates\FWdummy.bin bin\firmware.bin > NUL
copy /y templates\BNdummy.bin bin\bn.bin > NUL
..\tools\sfk partcopy bin\output.bin -fromto 0 -1 bin\firmware.bin 512 -yes > NUL
...
:END
copy bin\firmware.bin firmware.bin
ECHO *** Done. ***
